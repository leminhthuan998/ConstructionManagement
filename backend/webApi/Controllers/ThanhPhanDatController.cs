...
{
    [Authorize]
    [Route("/api/thanh-phan-dat")]
    public class ThanhPhanDatController : ControllerBase
    {
        private readonly ApplicationDbContext _dbContext;
        private readonly DbSet<ThanhPhanMeTronDat> _repository;

        public ThanhPhanDatController(ApplicationDbContext dbContext)
        {
            this._dbContext = dbContext;
            _repository = _dbContext.Set<ThanhPhanMeTronDat>();
        }

        [HttpGet("index")]
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(ApiResponse<List<ThanhPhanMeTronDat>>))]
        public async Task<IActionResult> IndexAction()
        {
            var results = await _repository.ToListAsync();
            var newRs = new List<ThanhPhanMeTronDat>();
            ...
            return Ok(ApiResponse<List<ThanhPhanMeTronDat>>.ApiOk(newRs));
        }
    }
}