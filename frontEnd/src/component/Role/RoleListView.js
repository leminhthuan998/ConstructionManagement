...
import { API_ROLE_DELETE, API_ROLE_DETAIL, API_USER_DETAIL, API_ROLE_USER } from '../../constants/ApiConstant';
...
class RoleListView extends Component {
  ...
  //edit
  openFormEdit(data) {
    console.log("RoleListView -> openFormEdit -> data", data)
    Axios.get(AppUtil.GLOBAL_API_PATH + API_ROLE_USER + `?roleId=${data.id}`)
      .then(res => {
        const { data } = res;
        console.log("FormUpdate -> openFormEdit -> data", data)
        if (data.success) {
          this.setState({
  ...
  onConfirmDelete(data) {
    const params = {}
    params['id'] = data.id
    Axios.post(AppUtil.GLOBAL_API_PATH + API_ROLE_DELETE, null, {
      params
    })
      .then(res => {
        const { data } = res;
  ...
  loadData() {
    ...
    console.log("loadData -> componentDidMount -> rowSelect", this.state.rowSelect)

    Axios.get(AppUtil.GLOBAL_API_PATH + API_ROLE_DETAIL)
      .then(res => {
        const { data } = res;
        if (data.success) {
          this.setState({
  ...
  render() {
    // console.log("rowSelect", this.state.rowSelect)
    // console.log("rowData", this.state.rowData)
    // console.log("listview", this.state.dataUserRole)
    return (
      <div className="ag-theme-alpine" style={{
        ...
        >
          <FormUpdate ref={c => this.formUpdate = c}  dataUserRole={this.state.dataUserRole} dataUser={this.state.dataUser} data={this.state.rowSelect}
            loadData={() => this.loadData()} />
          {/* {console.log("RoleListView -> render -> this.state.dataUserRole", this.state.dataUserRole)} */}
        </Modal>
        <Modal
          title="Tạo mới"
        ...
        >
          <FormUpdate create ref={c => this.formUpdate = c} data={this.state.rowSelect} dataUser={this.state.dataUser}
            loadData={() => this.loadData()} />
        </Modal>
      </div>
    );
